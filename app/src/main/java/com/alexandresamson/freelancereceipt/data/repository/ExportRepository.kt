...
class ExportRepository(private val context: Context) {
    ...
    suspend fun exportCsv(receipts: List<ReceiptEntity>): Intent =
        withContext(Dispatchers.IO) {
            ...
            FileWriter(file).use { writer ->
                writer.write("﻿")
                writer.write(context.getString(R.string.export_csv_header) + "\n")
                receipts.forEach { r ->
                    val categoryName = Category.dbKeyToDisplayName(context, r.category)
                    writer.write(
                ...
                writer.write("\n;${context.getString(R.string.export_doc_total)};${"%.2f".format(total)};;\n")
            }
    ...
    suspend fun exportPdf(receipts: List<ReceiptEntity>): Intent =
        withContext(Dispatchers.IO) {
            ...
            val primaryColor = Color.rgb(33, 97, 140)
            ...
            canvas.drawText(context.getString(R.string.export_doc_title), margin, 38f, paintText)

            paintText.textSize = 10f
            paintText.isFakeBoldText = false
            canvas.drawText(
                context.getString(R.string.export_doc_exported_on, dateFormat.format(Date())),
                margin, 58f, paintText
            )
            canvas.drawText(
                context.getString(R.string.export_doc_receipt_count, receipts.size),
                margin, 70f, paintText
            )
            ...
            val headers = arrayOf(
                context.getString(R.string.export_doc_header_date),
                context.getString(R.string.export_doc_header_merchant),
                context.getString(R.string.export_doc_header_amount),
                context.getString(R.string.export_doc_header_tax),
                context.getString(R.string.export_doc_header_category)
            )
            val rowH = 22f
            ...
            receipts.forEachIndexed { index, receipt ->
                ...
                val categoryName = Category.dbKeyToDisplayName(context, receipt.category)
                val cells = arrayOf(
            ...
            paintText.textSize = 9f
            canvas.drawText(
                context.getString(R.string.export_doc_total_gross),
                colX[1] + 4f, y + 16f, paintText
            )
            ...
            canvas.drawText(
                context.getString(R.string.export_doc_net, currencyFormat.format(netTotal)) +
                        "   " +
                        context.getString(R.string.export_doc_tax, currencyFormat.format(taxTotal)),
            ...
            canvas.drawText(
                context.getString(R.string.export_doc_footer, pageNumber),
